...
public class GameSetting : MonoBehaviour
{
    ...
    public void LoadAudioVolume() //讀取音樂設定
    {
        ...
        gameSFXVolumeSlider.value = PlayerPrefs.GetFloat("SFXVolume",1);
        SetMainVolume();
        SetGameVolume();
        SetGameSFXVolume();
    }
    ...
    void SetResolution(int i)
    {
        ...
        bool isFull;
        if (PlayerPrefs.GetInt("FullScreen",1) == 1) isFull = true; else isFull = false;
    ...
    void SetResolutionInfo()
    {
        ...
        for (int i = 0; i < fliterResolution.Count; i++)
        {
            ...
            if (fliterResolution[i].width == PlayerPrefs.GetInt("ScreenResolutionX", Screen.currentResolution.width)
                 && fliterResolution[i].height == PlayerPrefs.GetInt("ScreenResolutionY", Screen.currentResolution.height))
            {
                currnetIndex = i;
            }
        }
        ...
        //設定isFull 的Toggle
        if (PlayerPrefs.GetInt("FullScreen", 1) == 1) fullToggle.isOn = true; else fullToggle.isOn = false;
    }
    //--------------------------解析度設定--------------------------//
}
